        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-xl p-6">
                    ...
                    <div className="text-gray-400">{t('home.techArticles')}</div>
                </div>
                <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/20 rounded-xl p-6">
                    <div className="text-3xl font-bold text-purple-400 mb-2">5</div>
                    <div className="text-gray-400">{t('home.techCategories')}</div>
                </div>
                <div className="bg-gradient-to-br from-pink-500/10 to-pink-600/10 border border-pink-500/20 rounded-xl p-6">
                    ...
                    <div className="text-gray-400">{t('home.totalReads')}</div>
                </div>
            </div>
                ...
                <div className="space-y-6">
                    {posts.map((post, index) => (
                        <Link key={post.slug} href={`/posts/${post.slug}`} className="block group">
                            <article className="relative bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6 pl-8 md:pl-6 hover:border-blue-500/50 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300">
                                <div className="absolute left-1 md:-left-3 top-6 w-6 h-6 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                    {index + 1}
                                </div>
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                    <div className="flex-1">
                                        <h3 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors mb-2">{post.title}</h3>
                                    ...
                                    <div className="flex items-center text-blue-400 group-hover:translate-x-2 transition-transform">
                                        <span className="text-sm font-medium">{t('home.readMore')}</span>
            ...
            {posts.length === 0 && (
                <div className="text-center py-20">
                    ...
                    <h3 className="text-xl font-semibold text-gray-400 mb-2">{t('home.noArticles')}</h3>
                    <p className="text-gray-500">{t('home.noArticlesDesc')}</p>
                </div>
            )}
        </div>
    )
